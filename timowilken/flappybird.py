...
class PipePair(pygame.sprite.Sprite):
    ...
    @property
    def bottom_y(self):
        """Get the y of the bottom pipe"""
        return WIN_HEIGHT - self.bottom_height_px

    @property
    def middle_y(self):
        """Get the y of the between pipe"""
        mid_dist = (self.bottom_y - self.top_height_px) / 2
        return self.top_height_px + mid_dist
...
def playGame(status2, qmodel):
    score = 0
    tries = 1
    all_score = 0
    max_score = 0
    target = open('data/status.txt', 'w')
        ...
        while not done:
            ...
            if (frame_clock % 12 == 0):
                if next_pipe is not None:
                    y = math.floor(next_pipe.bottom_y - bird.y)
                    if bird.y < WIN_HEIGHT / 4:
                        quadrant = 0
                    elif bird.y < WIN_HEIGHT / 2 and bird.y >= WIN_HEIGHT/4:
                        quadrant = 1
                    elif bird.y < WIN_HEIGHT * 3/4 and bird.y >= WIN_HEIGHT / 2:
                        quadrant = 2
                    else:
                        quadrant = 3

                    state = (quadrant, math.floor(next_pipe.x - bird.x), y)
                    # print ('bird: ', bird.y, bird.x, 'pipe: ', next_pipe.x, next_pipe.bottom_height_px, next_pipe.top_height_px)
                else:
                    state = (0, 0, 0)

                curr_state = state
                qmodel.updateAction((old_state, old_action), curr_state, reward)

                if reward < 0:
                    reward = 1

                last_status = ((old_state, old_action), curr_state)

                old_action = False

                if (qmodel.getAction(curr_state)):
                    ...
                    old_action = False
                if WRITE:
                    qmodel.writeValues(old_state, old_action, curr_state, reward)
                # qmodel.printValues(old_state, old_action, curr_state, reward)

                old_state = curr_state

            if reward < 0:
                qmodel.updateDeath(last_status[0], last_status[1], reward)
                reward = 1
            ...
            for i, p in enumerate(pipes):
                if p.x + PipePair.WIDTH < bird.x and not p.score_counted:
                    ...
                    if (tries % 7 == 0):
                        target.seek(0)
                        target.write(str(all_score / tries))
...
if __name__ == '__main__':
    ...
    status2 = open('data/status2.txt', 'w')
    qmodel = Qmodel(status2, (0, 0, 0))
    playGame(status2, qmodel)
